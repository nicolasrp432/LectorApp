...
const BottomNav: React.FC<BottomNavProps> = ({ currentRoute, onNavigate }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-background-dark border-t border-gray-200 dark:border-card-border pb-safe">
      <nav className="flex items-center justify-between h-16 px-2 max-w-md mx-auto">
          ...
          >
            ...
            {isDashboard && <div className="absolute top-0 w-8 h-1 bg-primary rounded-b-full shadow-[0_2px_10px_rgba(52,211,153,0.5)]"></div>}
          </button>
          ...
          <div className="relative -top-6">
            <button 
              ...
              className="flex items-center justify-center size-16 rounded-full bg-primary text-background-dark shadow-glow hover:shadow-glow-lg hover:scale-105 transition-all active:scale-95 border-4 border-white dark:border-background-dark"
            >
      ...
      <div className="h-[env(safe-area-inset-bottom)] w-full bg-white dark:bg-background-dark"></div>
    </div>
  );
};
